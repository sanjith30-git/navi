...
export const NavigationBot: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Initial welcome message (text only, no auto-speech)
    ...
    // Removed auto-speech functionality
      ...
      <div className="max-w-7xl mx-auto px-4 py-6 pb-24 relative z-20">
        <div className="grid gap-6 lg:grid-cols-1">
          ...
          <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
            {/* Bot Avatar Section */}
            <div className="bg-gradient-to-r from-blue-600 to-purple-600 p-8 flex flex-col items-center justify-center min-h-[300px]">
              <BotAnimation 
                isSpeaking={navState.isSpeaking}
                isListening={navState.isListening}
              ...
              <p className="text-blue-100 text-sm mt-1">
                {navState.isListening ? getTranslation('listening', navState.language) : 
                 ...
                 (speechManagerRef.current && speechManagerRef.current.queueLength > 0) ? 
              ...
              {/* Button to trigger bot speech */}
              <button
                onClick={() => {
                  const welcomeMessage = getTranslation('welcome', navState.language);
                  speakMessage(welcomeMessage);
                }}
                disabled={navState.isSpeaking || navState.isListening}
                className="mt-4 px-6 py-3 bg-white bg-opacity-20 hover:bg-opacity-30 text-white rounded-full font-medium transition-all duration-300 transform hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed flex items-center space-x-2"
              >
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.536 8.464a5 5 0 010 7.072m2.828-9.9a9 9 0 010 12.728M5.586 15H4a1 1 0 01-1-1v-4a1 1 0 011-1h1.586l4.707-4.707C10.923 3.663 12 4.109 12 5v14c0 .891-1.077 1.337-1.707.707L5.586 15z" />
                </svg>
                <span>{navState.language === 'tamil' ? 'பேசுங்கள்' : 'Speak'}</span>
              </button>
            </div>
